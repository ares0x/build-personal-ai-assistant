...
- **Harness（缰绳）**：我们重点演示的是大模型的外围控制层（Harness），即大模型如何接收输入、在什么时机决定调用工具、如何向系统返回响应。
...
对于初学者来说，成熟的 Agent 框架（如 LangChain、LlamaIndex）内部封装了极其厚重的层级结构和隐式调用。直接使用它们虽然能快速拼装应用，但很难帮我们看清以下底层技术细节：
...
- **运行时**：Node.js $\ge 20$ (利用其成熟的 ESM 支持)。
