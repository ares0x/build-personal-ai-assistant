---
title: 03. 最小可用核心 ReACT 循环与 Mock LLM | hachimi 教程
description: 本章带你使用原生 TypeScript 手写第一个 Agent ReACT（思考-行动）执行循环，定义 Tool/Agent 契约并通过本地 Mock 流程测试其控制逻辑。
keywords: ReACT 循环, Agent Loop, MockLLM, Tool 契约, 控制流程, Node.js Agent
---
